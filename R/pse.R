library(sensitivity)
# Para FAST99
error.bar <- function(x, lower, upper){
		arrows(x,lower, x, upper, angle=90, code=3, length=0.1)
}
myplot <- function(x, ylim = c(0, 1), ...) {
		...
		if (class(x)=="sobol2007") {
				...
				S[S<0] <- 0;
				colnames(S) <- colnames(x$X)
				e<-barplot(S, ylim= ylim, col=bar.col)
				error.bar(e, x$S$"min. c.i.", x$S$"max. c.i.")
				error.bar(e, x$T$"min. c.i.", x$T$"max. c.i.")
		}
}
