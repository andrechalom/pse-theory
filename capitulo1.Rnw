...
%GA: sensitividade a p. mut, p. crossov, elitismo, num de genomas, num de its
 % 4 MaxEnt: lhs dos betas - capitulo 2?
...
% Inputs explicitos VS inputs implicitos (grain!)
%TODO Modelos de tradeoffs x coexistencia. r-K continuum revisoes
...
subset of this space that respects all our constraints. We 
will return to questions related to constrained parameter
...
A similar problem is to determine simply which parameters play any role on 
the model output. This simpler problem will be addressed by Morris screening
in section \ref{Morris}.
...
@
	\end{center}
	\caption{Four different possible origins from $D_i$. Pannel ``a'' shows 
...
a <-1; K <-0.1;
par(mfrow=c(2,2))
plot(x[ x+a*y < K], y [ x+a*y < K], main="x+ay < K", xlab="", ylab="")
plot(x[ x * y < K], y [ x * y < K], main="xy < K", xlab="", ylab="")
plot(x[ x * y > K], y [ x * y > K], main="xy > K", xlab="", ylab="")
plot(x[ x / y < K], y [ x / y < K], main="x/y < K", xlab="", ylab="")
@
	\end{center}
	\caption{Four samples of constraints on the parameter space,
	and the general shape imposed by those constraints.}
	\label{fig:TDi}
\end{figure}
...
% Lotka Volterra generalizado, adicionando species - vc consegue montar comunidades robustas com mtas especies
% LHS com n especies, forca de interacao (variancia da normal de media 0), etc
...
\begin{itemize}
		...
		\item Monotonic relation, which can be tested with the Spearman partial correlation coefficient (fig. \ref{fig:PRCC}),
		\item Trends in central location, for which the Kruskal-Wallis test may be applied (fig. \ref{fig:Kruskal}),
		\item Trends in variability, for which the FAST method and Sobol' indices may be used (fig. \ref{fig:eFAST}),
...
par(mfrow=c(2,2))
myplot<- function() {
		plot(pcc(data.frame(r,k,Time,x), res, nboot=100, rank=TRUE))
		mtext(paste('n = ',N), at=3.5)
...
@
	\end{center}
	\caption{PRCC coefficients for 4 input variables on a model. The vertical
	bars indicate the confidence interval for each value, calculated by
	bootstraping.The four panels present results from different sample sizes.}
	\label{fig:PRCC}
\end{figure}
...
\input revlit.tex % Revisao da literatura
