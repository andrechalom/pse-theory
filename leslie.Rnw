...
classes. The
matrix used on the density-independent model is, with $P_i$ being the 
...
As usually done on this modelling approach, the dominant eigenvalue 
$\lambda$ was calculated for this matrix. This value is related to the
...
assuming a normal distribution of parameters. In the case of the density dependence
...
playing little role. In the density-dependent model, this pattern is not evident. Instead, the next most important parameter after $F_7$ seems to be
...
for each model and run the models for each of the 650 combinations of parameters. The 
...
for the majority of the model output, in particular interactions involving $S_7$, the
...
TODO: discutir esses resultados
