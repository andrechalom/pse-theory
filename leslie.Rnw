...
By using SBMA (section \ref{SBMA}), we have determined that the sample size
required for the density independent model is approximately 200, 
and 500 for the density dependent.

<<table1, fig=FALSE, results=tex>>=
print(xtable(prcctab, caption='Comparison of PRCC analyses by sample size for both models'))
@
...
model, the mean stable population predicted 
(mean \Sexpr{round(mean(dLHS5@res))}, standard deviation \Sexpr{round(sd(dLHS5@res))}
sd trees in each $25m^2$ plot), 
is comparable to, altough slightly higher, the population  
...
plot(ecdf(LHS4), do.points=FALSE, main='')
mtext('(a)', at=0.95)
plot(ecdf(dLHS4), do.points=FALSE, main='')
mtext('(b)', at=-500)
@
		\end{center}
