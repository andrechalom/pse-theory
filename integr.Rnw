\section{Motivação}
O desenvolvimento e utilização de modelos matemáticos e computacionais deve encompassar % anglicismo
as seguintes etapas: 
...
um sistema. % Antes de enumerar as vantagens cabe uma definição desta abordagem
...
\begin{enumerate}
	...
	\item O princípio de verossimilhança afirma que toda a informação contida nas amostras coletadas está contida 
		...
		levar à falsa impressão de gerar respostas com maior precisão do que a informação coletada permite. % aqui há dois aspectos: o uso da informação de incerteza ou variância contida na amostra, o que pode ser feito de várias maneiras, e justificar o uso da funçãod e verosimilhnaça como a maniera mais adequada de se fazer isso.
\end{enumerate}
...
{\mathcal{L}(\theta_2|\bu{x})}$. % qual o propósito deste parágrafo?
...
Como exposto na sessão \ref{sec:likelihood}, a lei da verossimilhança (LL, do inglês Law of Likelihood),
...
Embora essa proposta seja valiosa quando possível, ela não é factível em uma larga classe de modelos, e é embasada
em aproximações.
% Ainda não está claro para mim que analisar a função de verossimilhança com muito parâmetros (e.g. por meio de perfilhamento) é o mesmo problema que construir hipóteses compostas. Lembre-se de me explicar esta parte.
...
\cite{Zhang09} e \cite{Bickel10}, por exemplo. Zhang considera duas hipóteses, $H_1 : \theta \in \Theta_1 \subset \Theta$
...
% Intuição: se o problema é apenas esse pode existir uma outra operação de conjuntos que faça o papel da multiplicação para as hipóteses simples. Faz sentido?
...
e é o caminho que vamos seguir nesta sessão. % isso é o mesmo que fomular uma lei fraca?
...
% A proposta faz sentido para mim, mas depois de ler as seções anteriores fiquei em dúvida sobre a interpretação. Creio que isso acontece porque ainda não há uma ligação clara entre a teoria das seções anteriores a aplicação e exemplos que começma aqui, confere?
...
\end{enumerate}
% Tive dificuldades de identificar estes passos nos exemplos. No
