...
Considere a função $\Psi (\gamma | \bu{x})$ definida por:
\begin{equation}
...
nos diz que o suporte para esse valor de $\gamma$ é a soma do suporte dado aos valores de $\theta$. Vamos mostrar a seguir que
a função $\Psi$ satisfaz os requerimentos de uma medida de suporte (ver \cite{Edwards72}, sec. 3.2):
