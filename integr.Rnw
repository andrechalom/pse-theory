...
simples $H_p:\theta=p , \, p \leq 0.2$ é de no mínimo 91.4. Todos esses cálculos sugerem que os dados sejam coerentes com
...
atribuam um único valor de probabilidade às observações da variável aleatória sob consideração. É importante notar que
...
O suporte absoluto a um único valor de parâmetro pode ser infinitesimal - mas o mesmo racional pode ser empregue
sobre uma região $x+dx$, construindo uma função que atribui a cada intervalo uma densidade de suporte absoluto.
