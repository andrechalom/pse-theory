...
resolvendo as equações corretamente \cite{ASC2010}.
...
essencial para que o modelo possa ser considerado para fins preditivos \cite{ASC2010}.
...
em capítulos diferentes dos livros \cite{Caswell89}, e frequentemente realizados por equipes diferentes. Em estudos que 
empregam modelos matriciais, a discussão dos resultados se concentra no ponto obtido por uma combinação 
de parâmetros considerada ótima durante as estimativas de parâmetros \cite{SilvaMatos99}, e a análise de incerteza é apresentada
...
Já a teoria analítica de AIS, devida em grande parte aos trabalhos de Hal Caswell \cite{Caswell89}, procede rotineiramente por
...
\section{Uma função de suporte}\label{sec:pseudo}
...
modelo, a partir de um conjunto de dados coletados?''. Sabemos que essa pergunta, se formulada sobre os parâmetros
...
$\mathcal{L}(\phi|\bu{x}) = \mathcal{L} \left(f(\theta)|\bu{x}\right)$ (\cite{Edwards72}, sec. 2.5).
Vamos estender esse resultado para uma função genérica. Nosso argumento se baseia em uma função de um argumento,
$\gamma = g(\theta)$, sendo que a generalização para mais dimensões é trivial.

Vamos construir uma função de pseudoverossimilhança $\Psi (\gamma | \bu{x})$, definida por:
\begin{equation}
\Psi ( \gamma | \bu{x} ) = \int_{g(\theta) = \gamma} \mathcal{L} (\theta | \bu{x}) \,d\theta
\label{eqn:Psi}
\end{equation}

Intuitivamente, podemos considerar que se dois valores de $\theta$ levam a um mesmo valor de $\gamma$, a equação \ref{eqn:Psi}
nos diz que o suporte para esse valor de $\gamma$ é a soma do suporte dado aos valores de $\theta$. No entanto, a 
verossimilhança, ao contrário da probabilidade, não é aditiva, de forma de a função $\Psi$ não é uma função de verossimilhança. 
Vamos mostrar a seguir que as condições nas quais a função $\Psi$ satisfaz os requerimentos de uma 
medida de suporte (ver \cite{Edwards72}, sec. 3.2):
...
O problema de definição de uma pseudoverossimilhança para análise de incerteza em modelos está intimamente ligada à questão
de hipóteses compostas sob o paradigma da verossimilhança. 
...
\section{Um adendo Bayesiano}
Do ponto de vista Bayesiano, podemos enxergar a função de probabilidade {\em a posteriori} de $\gamma$ como:

O que explica porque a propriedade de invariância a transformações nos parâmetros (sec. \ref{sec:pseudo}) não é mantida por
...
\begin{enumerate}
	...
	\item Caso haja um modelo cujo desempenho é inequivocamente superior, use o método de Metropolis (forma de Monte Carlo) para gerar amostras a partir da função de verossimilhança do modelo. A distribuição de resultados $y$ do modelo pode ser vista como a função de verossimilhança de $y$, já que $y=f(x)$ (ver \cite{Edwards72}, sessão 6.5?)
	\item Caso haja um empate entre modelos, gere amostras a partir de todos os modelos vencedores e as combine usando os pesos designados pela diferença de informação (ver \cite{Edwards72}, teorema 5.4.2?).
