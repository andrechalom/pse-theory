...
estão muito interligadas, e costumam ser referidas como V \& V: a verificação consiste em garantir que o código 
...
como um procedimento separado e posterior à análise do resultado ``principal''. Até mesmo a abordagem estatística usada
em ambos os procedimentos pode ser incoerente.
