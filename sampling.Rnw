...
relevant (but see \cite{Morris91} for an alternative). 
...
As shown on \cite{McKay79}, the estimatives of statistical properties (such
...
given for simplicity; see \cite{Huntington98} for an alternative with better numerical
properties}:
...
from McKay \cite{McKay79}, described in the previous section,
...
on the example on section \ref{IBM}. 
