...
#library(mcmc)
#atual <- as.numeric(x[1,])
#m <- metrop(Met.f, atual, 99999, blen=5)
#dis <- m$batch
...
de $\lambda$. A distribuição dos valores de $\lambda$ foi usada como aproximação para a função de suporte de $\lambda$,
exibida na figura \ref{fig:lambda}. % PI: Isso é uma função de suporte ou uma distribuição a posteriori
A máxima densidade dessa distribuição é atingida em 
$\lambda = $ \Sexpr{round(d$x[which(d$y == max(d$y))],2)}. As figuras \ref{fig:lambdascatter} e \ref{fig:lambdaprcc} mostram
resultados preliminares da aplicação de técnicas de análise de sensibilidade sobre as amostras geradas.
% PI: O significado desses resultados ainda não estão claros para mim.
% C: Nem pra mim, pra falar a verdade. Mas não acho q eu possa ser tão sincero por escrito :P

Futuramente, serão realizadas análises mais aprofundadas sobre esse exemplo.

\begin{figure}
<<Case1>>=
plot(density(dis[,4]))
@
	\caption{Distribuição dos resultados do modelo obtidos amostrando-se a função de verossimilhança dos parâmetros.}
	\label{fig:lambda}
\end{figure}
\begin{figure}
<<>>=
