...
%%PI: a partir daqui avaliar o que vai para um apêndice. A informação
%%essencial a reter é que lambda é um parametro derivado obtido pela
%%maximização das somas das log-verossimilhanças 
%% (detalhe técnico: sob a forte premissa de que os parâmetros são
%% independentes neste exemplo, para simplficar, acho que isso vai apêndice).
...
adultos recém maduros, \Sexpr{obs[2]} nascidos e \Sexpr{obs[3]} sobreviventes. É fácil ver 
%% Pi: agora que omitiu as expressões será fácil ver na figura, o que muda o início do parágrafo seguinte.
...
foram utilizadas para gerar \Sexpr{get.N(plue)} amostras pelo método de Metropolis, a partir das quais calculou-se o valor
de $\lambda$. 
%% PI: este é segundo passo crucial do método, uma vez definida a função de verossimilhança
%% Com isso geramos uma distribuição empírica de lambda que ...
%% Seria pq o MCMC explora o epaço de parametros proporcional ao suporte?
%% Um mérito importante é que o PLUE pode ser visto como uma análise de sensibilidade
%% em uma vizinhança não infinetesimal e não arbitrária, e de uma quantidade derivadas de uma função não monotônica dos mles
...
discutidas no capítulo \ref{cap:pse}.
