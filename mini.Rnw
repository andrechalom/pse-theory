...
exibida na figura \ref{fig:lambda}. % Isso é uma função de suporte ou uma distribuição a posteriori
...
% O significado desses resultados ainda não estão claros para mim.
...
plot(density(dis[,4]))
@
	\caption{Distribuição dos resultados do modelo% obtidos amostrando-se a função verossimilhança ...}
	\label{fig:lambda}
\end{figure}
\begin{figure}
<<>>=
