...
intuitivamente atraente dentro de um paradigma de verossimilhança, além de ser coerente com a 
Lei Geral da Verossimilhança (GLL) enunciada acima, para uma dada escolha de hipóteses.
...
Essas perguntas não podem ser respondidas de um ponto de vista verossimilhantista clássico, por corresponderem
...
do ponto de máximo global ($x_{max}$) com o máximo obtido em uma região distinta ($x_{lim}$). 
...
@
	\caption{Representação esquemática de uma função de verossimilhança, veja detalhes no texto.} 
	\label{fig:esquema}
\end{figure}

\subsection{Método}

Seja $y = F(\bu{x})$ o resultado escalar de um modelo 
