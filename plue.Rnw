...
de perfilhamento vêm sendo utilizados na inferência por verossimilhança há decadas para reduzir a 
...
\begin{description}
	\item[Definições]
		...
		de um resultado escalar $y$ de um modelo com um vetor de entradas $\bu{x}$: $y = F(\bu{x})$ 
	\item[Amostragem]
