...
will present examples of use of those tests.
...
\subsubsection{Trends in central location}
...
model runs and applying one of the techniques outlined on the previous 
subsections. If the analyses indicate similar results for consecutive
experiments, we can presume that increasing the sample size will not
yield major changes to the results.

Both PRCC and eFAST analyses present us with a list of the parameters 
...
We will apply SBMA for the example on section \ref{Leslie}. When comparing
ranks for PRCC analysis, it should be understood that we are comparing the
