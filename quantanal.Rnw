...
The classical approach to the sensitivy analysis, based on the frequentist
...
\begin{itemize}
	...
	\item Monotonic relation, which can be tested with the Spearman partial correlation coefficient,
		also refered to as Partial Rank Correlation Coefficient, or PRCC.
		This measure is a robust
		indicator of monotonic interactions between $y$ and $x_i$, and is subject
		to significance testing as described in \cite{Marino08}.
	\item Trends in central location, for which the Kruskal-Wallis test may be applied (\cite{Kleijnen99}).
	\item Trends in variability, for which the FAST method and Sobol' indices may be used in order to partition
		the model variability (\cite{Archer97, Saltelli99, Saltelli04}).
\end{itemize}
...
between the $y$ conditional to each class \cite{Kleijnen99}.
...
for Generalized Likelihood Uncertainty Estimation\cite{Beven92}. While
...
formal Bayesian approaches have been developed\cite{Vrugt09}.
Another approach, based on the Metropolis algorithm, is provided by 
Kuczera\cite{Kuczera98}. %TODO
