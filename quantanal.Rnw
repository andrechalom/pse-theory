...
when enough correlation terms have been taken into account. Smith (\citep{Smith02})
...
\begin{itemize}
	...
	\item Monotonic relation, which can be tested with the Spearman partial correlation coefficient,
		...
		to significance testing as described in \citep{Marino08}.
...
indicator of monotonic interactions between $y$ and $x_i$, and is subject
to significance testing as described in \citep{Marino08}. This measure will 
