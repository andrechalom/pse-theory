...
set.seed(17)
@
Chalom, A. \footnote{Theoretical Ecology Lab, LAGE at  Dep. Ecologia, Instituto de Biociências, Universidade de São Paulo,
...
(available on http://cran.r-project.org/web/packages/pse). This package is based
...
A discrete uniform density function, useful for parameters that
...
\section{Your model}
The model that you wish to analyze must be formulated as an \R function that
...
This is a well known behavior of many population models
...
coefficients of two runs with different sample sizes. 
<<sbma>>=
...
using the ``index.res'' parameter, and suppress the
linear model from being plotted with the parameter ``add.lm'':
<<corplot2, fig=T>>=
